...
//======================= 先初始化資料============================//
if (wantList) {
  var wantList = wantList.split(',') //得到目前有的清單
  var sum = 0  //後面算合計用
  wantList.forEach(function (elm, idx) {
    ...
    var $ul = $(
      ...
      <li ><input type="number" value=${data.count} id="slide_buycart_count" "></input></li>
      <li id="slide_buycart_total"align="center">NT$ <span>${data.singlePrice}</span></li>
    ...
    //算出產品小計
    good_singleTotal = data.count * data.singlePrice
    sum += good_singleTotal
    //先把已經有的品項放進購物車
  ...
  $('.fa-cart-shopping').on('click', function () {
    ...
    var nameArr = new Array(); //定義nameArray為新陣列(把原本存在的先放進去，後面用於檢查)
    ...
    if (isHasName >= 0) { //被點及一次後，就會往上加，初始為-1
      ...
      //更新localStorage的東西
      var goodJson = myStorage.getItem(goods_name)
      var goodsData = JSON.parse(goodJson)
      // console.log(goodsData.count)
      goodsData.count ++;
      goodsData.totalPrice =goodTotal;
      myStorage.setItem(goods_name, JSON.stringify(goodsData))
